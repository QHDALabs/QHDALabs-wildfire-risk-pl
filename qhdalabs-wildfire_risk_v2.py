...
def qaoa_sensor_placement(results: list, n_sensors: int = N_SENSORS) -> list:
    ...
    Problem: weighted maximum coverage — each cell has weight = risk score.
    We formulate as QUBO and solve with QAOA on a statevector simulator.

    Returns list of (lat, lon) for recommended sensor positions.
    Falls back to greedy top-N selection if Qiskit unavailable.
    """
    risks  = np.array([r["risk"] for r in results])
    coords = [(r["lat"], r["lon"]) for r in results]
    n      = len(results)

    # --- Greedy fallback (always computed, used if QAOA fails) ---
    ...
    try:
        ...
        linear = {f"x{i}": -float(risks[i]) for i in range(n)}
        qp.minimize(linear=linear)

        # Constraint: exactly n_sensors sensors
        ...
        sampler = QaoaSampler()
        log.info("QAOA: running optimizer (%d cells → %d sensors) …", n, n_sensors)
        ...
        if len(selected) != n_sensors:
            log.warning("QAOA returned %d sensors (expected %d) — using greedy.",
                        len(selected), n_sensors)
            return greedy_sensors

        sensors = [coords[i] for i in selected]
