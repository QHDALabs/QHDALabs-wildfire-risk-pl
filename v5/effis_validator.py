...
from pipeline_contract import StepStatus, write_step_manifest

_QUALITY_WARNINGS: list[str] = []


def _load_raster_runtime():
    try:
        numpy = importlib.import_module("numpy")
        rasterio_module = importlib.import_module("rasterio")
    except ModuleNotFoundError as exc:
        raise RuntimeError(
            "EFFIS was requested but Rasterio is unavailable. Install with: "
            f'"{sys.executable}" -m pip install -r requirements-effis.txt'
        ) from exc
    return numpy, rasterio_module
...
def inspect_tiff(path: Path) -> None:
    _, rasterio = _load_raster_runtime()
    print(f"\n{'=' * 60}")
    ...
    with rasterio.open(path) as src:
        ...
        # Podgląd zakresu wartości na ograniczonej próbce. Pełne rastry EFFIS
        # mogą mieć setki miliardów pikseli i nie mogą być ładowane do RAM.
        # Value-range preview on a bounded sample; never read the full raster.
        preview_height = min(src.height, 2048)
        preview_width = min(src.width, 2048)
        data = src.read(
            1,
        ...
        if len(valid) > 0:
            print("\n  Próbka wartości / Sampled values (fire pixels only):")
            print(f"    Min    : {valid.min():.4f}")
            print(f"    Max    : {valid.max():.4f}")
            print(f"    Mean   : {valid.mean():.4f}")
            print(f"    Nonzero in sample: {len(valid):,} px")
        else:
            print("\n  Brak pikseli z ogniem w próbce / No fire pixels in sample")
            if data.count():
                print(f"  Sample range: {data.min()} – {data.max()}")
...
def extract_per_nadlesnictwo(path: Path) -> dict:
    ...
    """
    np, rasterio = _load_raster_runtime()
    results = {}
        ...
        for name, (lat, lon) in NADLESNICTWA.items():
            try:
                if is_wgs84 or tiff_crs is None:
                    ...
                    xs, ys = transform("EPSG:4326", tiff_crs, [lon], [lat])
                ...
                _QUALITY_WARNINGS.append(
                    f"Pixel read failed for {name}: {type(exc).__name__}: {exc}"
                )
                results[name] = 0.0
...
def load_scores(scores_path: Path) -> dict:
    ...
    try:
        ...
        if isinstance(data, list):
            return {
                item.get("name", item.get("nadlesnictwo", "")): item.get("score", 0.0)
                for item in data
            }
        if isinstance(data, dict):
            if isinstance(data.get("scores"), list):
                return {
                    item.get("node_name", item.get("name", "")): float(
                        item.get("final_score", item.get("score", 0.0))
                    )
...
def compare_and_print(effis: dict, fusion: dict) -> None:
    ...
    """
    print(f"\n{'=' * 78}")
    print("  PORÓWNANIE / COMPARISON — EFFIS Severity 2025 vs Fusion SCORE")
    print(f"{'=' * 78}")
    print(
        f"  {'Nadleśnictwo':<22} {'EFFIS_raw':>10} {'EFFIS_norm':>10} "
    ...
    if fusion:
        ...
        if len(common) >= 5:
            effis_ranks = sorted(
                common, key=lambda n: effis_norm.get(n, 0), reverse=True
            )
            ...
            if rho > 0.7:
                print(
                    "  ✓ Silna zgodność rankingów — walidacja historyczna potwierdza system"
                )
            elif rho > 0.4:
                print("  ~ Umiarkowana zgodność — sprawdź węzły z największym Δ")
            else:
                print(
                    "  ✗ Słaba zgodność — wymaga analizy (lub 2025 nie miał pożarów na DS)"
                )

    print("\n  Uwaga: EFFIS severity 2025 może być zerowe jeśli sezon jeszcze trwa")
    print("  lub pożary na Dolnym Śląsku były poza progiem detekcji satelitarnej.")
    print("  Rozważ pobranie 2020-2024 dla pełnej walidacji historycznej.")
...
def main() -> int:
    # Windows may inherit a legacy CP-1250 console that cannot represent symbols
    # used by this bilingual report (for example Δ).
    ...
    if not tiff_path.exists():
        ...
        write_step_manifest(
            "topology",
            "effis",
            StepStatus.OPTIONAL_FAILED,
            valid_for_downstream=True,
            errors=[f"TIFF not found: {tiff_path}"],
        )
        return 1

    try:
        _load_raster_runtime()
    ...
    # Kroki 2+3
    print(f"\n{'=' * 60}")
    print("  Wyciągam wartości per nadleśnictwo...")
    print(f"{'=' * 60}")
    ...
    if nonzero == 0:
        ...
        print("  1. Plik TIFF obejmuje cały świat ale fire pixels są rzadkie")
        print("  2. Rok 2025 — sezon w toku, mało danych")
        print("  3. CRS pliku wymaga sprawdzenia (patrz wyżej)")
        print("\n  -> Pobierz severity_2022.tiff lub severity_2023.tiff")
        print("    (lata z udokumentowanymi pożarami na Dolnym Śląsku)")
    ...
    if scores_path.exists():
        ...
        print("  Użyj: py effis_validator.py tiff --scores topology/risk_scores.json")

    compare_and_print(effis_values, fusion_scores)
    write_step_manifest(
        "topology",
        "effis",
        StepStatus.DEGRADED if _QUALITY_WARNINGS else StepStatus.SUCCESS,
        inputs={"tiff": str(tiff_path), "scores": str(scores_path)},
        outputs={},
        coverage_percent=round(
            sum(value > 0 for value in effis_values.values())
            / len(effis_values)
            * 100.0,
            2,
        ),
        ...
        data_version="effis-validator-2.0.0",
    )
    return 0
